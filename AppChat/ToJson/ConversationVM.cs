...
{
    public class ConversationsToJson
    {
        public int Id { get; set; }
        public string User1Name { get; set; }
        public string User2Name { get; set; }
        public List<MessageVM> Messages;
        public static string GetJson(List<Conversation> cons)
        {
            var consToJson = new List<Conversation>();
            foreach (var con in cons)
            {
                var cvm = new ConversationsToJson();
                cvm.Id = con.Id;
            ...
            //sort conversations based on the first message that started them
            consToJson.Sort((x, y) =>
               DateTime.Compare(
                   ( DateTime.Parse(x.Messages[0].Timestamp)),
                   (DateTime.Parse(y.Messages[0].Timestamp)) )
               );

            return JsonConvert.SerializeObject(consToJson);
        }
    }
}
